...
KANBAN_GUIDANCE = (
    ...
    "7. **Commit your work before you finish.** If your workspace is a git "
    "worktree (or any tracked repo), `git add` and `git commit` before calling "
    "`kanban_complete` or `kanban_block`. Use a one-line imperative summary "
    "prefixed with the task id, e.g. `t_5c55e3c4: add assignee-vs-task-type "
    "validation`. For scratch workspaces (no git), list every file you wrote "
    "in `metadata[\"changed_files\"]`. Without a commit (or that manifest), the "
    "soft-complete scanner and the OYKAPY review workflow cannot find your "
    "deliverable — your work is invisible.\n"
    "    For code tasks against a tracked repo, prefer `--workspace worktree` "
